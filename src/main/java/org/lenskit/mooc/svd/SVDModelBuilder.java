...
public class SVDModelBuilder implements Provider<SVDModel> {
    ...
    public SVDModel get() {
        ...
        int nusers = userIndex.size();
        int nitmes= itemIndex.size();
        ...
        if (featureCount > 0) {
            ...
            userMatrix = userMatrix.getSubMatrix(0, nusers-1, 0, featureCount-1);
            itemMatrix = itemMatrix.getSubMatrix(0, nitmes-1,0, featureCount-1);
    ...
    private RealMatrix createRatingMatrix(KeyIndex userIndex, KeyIndex itemIndex) {
        ...
        try (ObjectStream<Rating> ratings = dao.query(Rating.class)
            ...
            for(Rating rating : ratings){
                ...
                int itemIdx = itemIndex.getIndex(itemId);
                int userIdx = userIndex.getIndex(userId);
                matrix.setEntry(userIdx, itemIdx, rating.getValue()
                        -intercept
                        -baseline.getItemBias(itemId)
                        -baseline.getUserBias(userId));
            }
        }
